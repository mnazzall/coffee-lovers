import React, { useEffect } from "react";
import "./History.css"


function History() {
    
    return (
        <div className="history-container">
    
        <div className="history-hero-container">
            <img src="/coffee-lovers/photos/historyherosection.jpg" alt="Ethiopian coffee farmer" />
            <h1>From Wild Berries to Global Ritual: The Journey of Coffee</h1>
            <p>Coffee’s history is as rich and complex as its flavor. From mystical legends to cultural revolutions, discover how this humble bean shaped the world.</p>
        </div> 

        <div className="history-timeline-container">
            <h1>The Timeline of Coffee</h1>
            <div className="shared-timeline">
                <h2>9th Century - Ethiopia: The First Discovery</h2>
                <ul>
                    <li><strong>Legend of Kaldi:</strong> An Ethiopian goat herder notices his goats dancing with unusual energy after eating red coffee cherries. Curious, he tries them himself.</li>
                    <li><strong>Early Use:</strong> Before brewing, coffee was often chewed or ground and mixed with animal fat for energy.</li>
                    <li><strong>Cultural Roots:</strong> The region of Kaffa is believed to give “coffee” its name. Coffee becomes part of Ethiopian oral tradition and hospitality rituals, including the beginnings of the Ethiopian coffee ceremony.</li>
                </ul>
            </div>
            <div className="shared-timeline">
                <h2>15th Century - Yemen: The Birth of Brewing</h2>
                <ul>
                    <li>Coffee crosses the Red Sea into Yemen, where Sufi monks begin brewing it as a hot drink to sustain long nights of prayer and meditation.</li>
                    <li><srong>Mocha Port:</srong> Yemen becomes the first hub of international coffee trade. The port city of Mocha gives its name to the famous “mocha coffee.”</li>
                    <li>Coffee’s identity shifts — from berry snack to spiritual beverage.</li>
                </ul>
            </div>
            <div className="shared-timeline">
                <h2>16th Century - Ottoman Empire: Coffeehouses Emerge</h2>
                <ul>
                    <li>Coffee reaches Istanbul (1555), carried by Ottoman traders.</li>
                    <li>The first kahvehane (coffeehouses) open, becoming centers of art, music, poetry, politics, and social life.</li>
                    <li>Coffee is so important that in some Ottoman marriage traditions, a bride’s skill at preparing coffee was seen as a test of her suitability.</li>
                    <li>Authorities try to ban coffeehouses at times, fearing their political influence — but the bans never last, showing coffee’s unstoppable popularity.</li>
                </ul>
            </div>
            <div className="shared-timeline">
                <h2>17th Century - Europe: Coffee Meets Controversy</h2>
                <ul>
                    <li>Coffee arrives in Venice (1615) via trade with the Ottomans.</li>
                    <li>Some clergy denounce it as the “bitter invention of Satan.” But Pope Clement VIII, after tasting it, blesses coffee instead of banning it.</li>
                    <li>
                        <strong>Coffeehouses spread across Europe:</strong>
                        <ul>
                            <li><strong>London:</strong> Known as “penny universities,” where ideas were exchanged over a single penny’s cup.</li>
                            <li><strong>Paris:</strong> Coffeehouses become meeting places for philosophers and writers.</li>
                            <li><strong>Vienna:</strong> Following the 1683 Battle of Vienna, coffee culture takes root with the invention of the Viennese coffeehouse tradition.</li>
                        </ul>
                    </li>
                </ul>
            </div>
            <div className="shared-timeline">
                <h2>18th Century - Colonial Expansion & Global Spread</h2>
                <ul>
                    <li>
                        <strong>Europeans smuggle coffee plants out of Yemen and cultivate them abroad:</strong>
                        <ul>
                            <li>Dutch establish plantations in Java (Indonesia).</li>
                            <li>French introduce coffee to the Caribbean.</li>
                            <li>Portuguese cultivate it in Brazil.</li>
                        </ul>
                        </li>
                    <li>Brazil’s ideal climate makes it the largest global producer, a position it still holds.</li>
                    <li>Coffee becomes deeply tied to colonialism and slavery, shaping entire economies and societies.</li>
                </ul>
            </div>
            <div className="shared-timeline">
                <h2>19th Century - Revolution & Industry</h2>
                <ul>
                    <li>
                        <strong>Coffeehouses fuel revolutions:</strong>
                        <ul>
                            <li>In France, cafés become centers of political debate before and during the French Revolution.</li>
                            <li>
                            In America, coffee rises in popularity after the Boston Tea Party, symbolizing independence from Britain.
                            </li>
                        </ul>
                        </li>
                    <li>During the Industrial Revolution, coffee becomes the drink of workers, providing fuel for long hours in factories.</li>
                    <li>Paris cafés inspire artists, poets, and intellectuals like Voltaire and Balzac, who famously consumed excessive amounts of coffee while writing.</li>
                </ul>
            </div>
            <div className="shared-timeline">
                <h2>20th Century - Coffee Reinvented</h2>
                <ul>
                    <li><strong>1901:</strong> Instant coffee is invented.</li>
                    <li><strong>1938:</strong> Nescafé launches, becoming especially popular during WWII.</li>
                    <li>Espresso culture emerges in Italy, giving birth to cappuccinos, macchiatos, and lattes.</li>
                    <li>Coffee becomes a modern commodity — global brands rise, especially Starbucks, spreading café culture worldwide.</li>
                </ul>
            </div>
            <div className="shared-timeline">
                <h2>21st Century - Coffee Today</h2>
                <ul>
                    <li>Coffee is cultivated in 70+ countries.</li>
                    <li>It’s the second most traded commodity after oil.</li>
                    <li>
                        <strong>The Third Wave Coffee Movement emphasizes:</strong>
                        <ul>
                            <li>Direct trade with farmers.</li>
                            <li>Sustainability.</li>
                            <li>Single-origin beans.</li>
                            <li>Artisanal brewing.</li>
                        </ul>
                        </li>
                    <li>Coffee continues to be a global ritual — from Ethiopian ceremonies, to Italian espresso bars, to specialty cafés in every major city.</li>
                </ul>
            </div>
        </div>
   
            <div className="darkside-of-history-container">
                <h1>Colonialism & Coffee’s Dark Side</h1>
                <ul>
                    <li>Coffee’s spread tied to European colonial powers.</li>
                    <li>Plantations in the Caribbean and Brazil built on slavery.</li>
                    <li>The legacy still shapes global trade and inequality.</li>
                </ul>
            </div>

            <div className="history-closing-container">
                <p>“From ancient rituals to modern cafés, coffee is more than a drink - it’s a shared human story.”</p>
            </div>


















           </div>
    );
}   


export default History;